...
The numeric totals are a historical baseline captured before later storage-service, bundled-viewer removal, and bundled tool/example migration work. Regenerate the counts before using them for current totals; the migration targets and contracts below remain authoritative.
...
| Existing `loadScanProject`/`saveScanProject` utility functions | `src/liblvr2/util/ScanProjectUtils.cpp`, `src/tools/lvr2_reconstruct/Main.cpp`, `src/tools/lvr2_scanproject_parser/Main.cpp` | Keep CLI behavior by delegating utilities to `lvr2::io::scan::load_project` / `save_project` until callers can use the new API directly. | Migrated to the ProjectStore helper path for the supported raw-PLY/HDF5 scan-project service surface; final CRTP deletion still owns unsupported raw-channel, generic array, camera/image, and mesh storage paths. |
