...
template<typename T>
std::string growingCellDebugSummary(const T& value)
{
    // Local adapter for stream-only GCS vector diagnostics.
    ...
    void GrowingCellStructure<BaseVecT, NormalT>::getMesh(HalfEdgeMesh<BaseVecT> &mesh){
        ...
        size_t runtime_length = (size_t)((((size_t)m_runtime*(size_t)m_numSplits)
                ...
                lvr2::log::info("{}", "[GCS Reconstruction] Starting Reconstruction");
        ...
        for(int i = 0; i < getRuntime(); i++)
        {
            ...
            ++monitor;
                ...
                lvr2::log::info("{}", "[GCS Reconstruction] Finished initial mesh");
        ...
        for(auto vertex : m_mesh->vertices())
        {
            performLaplacianSmoothing(vertex, m_mesh->getVertexPosition(vertex), 0.5); //no random point influence
                ...
                lvr2::log::info("{}{}", "Max depth of tt: ", (m_balances != 0 ? max_depth : tumble_tree->maxDepth()));
                lvr2::log::info("{}{}", "Not Deleted in TT: ", tumble_tree->notDeleted);
                lvr2::log::info("{}{}", "Tumble Tree size: ", tumble_tree->size());
                lvr2::log::info("{}{}", "KD-Tree size: ", kd_tree->size());
                lvr2::log::info("{}{}", "Cell array size: ", cellVecSize());
                lvr2::log::info("{}{}", "Not found counter: ", notFoundCounter);
                lvr2::log::info_runtime("");
                lvr2::log::info("{}{}", "Equilaterality test percentage: ", equilaterality().second);
                lvr2::log::info("{}{}", "Skewness test percentage: ", equilaterality().first);
                lvr2::log::info("{}{}", "Average Valence: ", avgValence());

                lvr2::log::info("{}{}", "Valances >= 10: ", numVertexValences(10));
                lvr2::log::info("{}{}", "Valances >= 15: ", numVertexValences(15));
        delete tumble_tree;
    }
    ...
    void GrowingCellStructure<BaseVecT, NormalT>::executeEdgeCollapse()
    {
        ...
        if(!m_useGSS)
        {
            ...
            else
            {
                                ...
                                lvr2::log::info("{}{}{}{}", "Lowest SC from Tumble Tree: ", min->signal_counter, " | ", (*min->duplicateMap.begin()).idx());
                                lvr2::log::info("{}{}", "Colapse threshold: ", m_collapseThreshold);
                ...
                if(min->signal_counter < this->getCollapseThreshold())
                {
                    ...
                    if(eToSixVal && m_mesh->isCollapsable(eToSixVal.unwrap()))
                    {
                        ...
                        cellArr[result.removedPoint.idx()] = NULL;
                                                lvr2::log::info("{}", "Collapsed an Edge!");
                    }
                }
            }
    ...
    void GrowingCellStructure<BaseVecT, NormalT>::initTestMesh(){
        ...
        auto pair = m_mesh->triCircumCenter(fH1);
                lvr2::log::info("{}{}{}{}", "CircumCenter1: ", detail::growingCellDebugSummary(pair.first), "| Radius: ", pair.second);
        auto pair1 = m_mesh->triCircumCenter(fH2);
                lvr2::log::info("{}{}{}{}", "CircumCenter1: ", detail::growingCellDebugSummary(pair1.first), "| Radius: ", pair1.second);
        auto pair2 = m_mesh->triCircumCenter(fH3);
                lvr2::log::info("{}{}{}{}", "CircumCenter1: ", detail::growingCellDebugSummary(pair2.first), "| Radius: ", pair2.second);
        auto pair3 = m_mesh->triCircumCenter(fH4);
                lvr2::log::info("{}{}{}{}", "CircumCenter1: ", detail::growingCellDebugSummary(pair3.first), "| Radius: ", pair3.second);
        auto pair4 = m_mesh->triCircumCenter(fH5);
                lvr2::log::info("{}{}{}{}", "CircumCenter1: ", detail::growingCellDebugSummary(pair4.first), "| Radius: ", pair4.second);
    ...
    void GrowingCellStructure<BaseVecT, NormalT>::getInitialMesh(){
        ...
        {
                        lvr2::log::info("{}", "Bounding Box invalid");
            exit(EXIT_FAILURE);
        }
        ...
        BaseVecT max = bounding_box.getMax();
                ...
                lvr2::log::info("{}{}", "Bounding Box min: ", detail::growingCellDebugSummary(min));
                lvr2::log::info("{}{}", "Bounding Box max: ", detail::growingCellDebugSummary(max));
        ...
        auto vH4 = m_mesh->addVertex(back);
                ...
                lvr2::log::info("{}{}{}{}{}{}{}", vH1.idx(), " | ", vH2.idx(), " | ", vH3.idx(), " | ", vH4.idx());
    ...
    void GrowingCellStructure<BaseVecT, NormalT>::removeWrongFaces()
    {
        ...
        avg_distance /= m_mesh->numVertices();
                ...
                lvr2::log::info("{}{}", "avg_distance to cloud: ", avg_distance);
        if(m_surface->get()->pointBuffer().get()->numPoints() < 10000000)         lvr2::log::info("{}{}", "avg distance between the points in the cloud: ", avgDistanceBetweenPointsInPointcloud());
    ...
    void GrowingCellStructure<BaseVecT, NormalT>::aggressiveCutOut(VertexHandle vH) {
                lvr2::log::info("{}", "Aggressive Cutout...");
