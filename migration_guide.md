...
## C++20 CLI parser and Boost program-options removal

Bundled LVR tools no longer use `boost::program_options` for command-line parsing. Tool option names, short aliases, defaults, positional inputs, and help/usage behavior are intended to stay compatible, but the public `lvr2::BaseOption` implementation now uses the small C++20 `lvr2::cli` parser in `<lvr2/config/CommandLine.hpp>`.

Downstream code that included `<lvr2/config/BaseOption.hpp>` only to derive tool options should not need a behavior change. Code that directly referenced Boost parser types from `BaseOption.hpp` must migrate to the LVR-owned names:

```cpp
#include <lvr2/config/CommandLine.hpp>

lvr2::cli::options_description options("Options");
lvr2::cli::variables_map variables;
```

`lvr2::cli::variables_map::count(name)` reports explicit command-line presence; defaulted values remain readable through `operator[]`. This matches the way existing LVR tool options use `count()` for flags, required positionals, and user-provided optional arguments. Help output keeps option names, aliases, value placeholders, defaults, and descriptions, but it is generated by the LVR parser rather than the removed Boost implementation.

The `boost-program-options` vcpkg port and Debian development package are no longer required by LVR tool parsing. Later Boost retirement work removed the remaining Boost package/export dependency surfaces as well.
...
Assimp is now part of the default vcpkg manifest dependency set because mesh asset I/O requires the private backend. The Assimp-backed mesh facade remains shared-only to avoid leaking private backend targets through exported static libraries. Static-first non-mesh/policy builds can configure with `-DLVR2_ENABLE_MESH_ASSET_IO=OFF` or `cmake --preset vcpkg-static-release`; in that lane `lvr2::io::mesh` load/save calls remain available but return structured `UnsupportedFormat` results for mesh asset formats. Optional vcpkg features must be requested alongside the matching CMake options for other packages. For example, use `-DVCPKG_MANIFEST_FEATURES=pcl -DLVR2_WITH_PCL=ON` for PCL tools and `-DVCPKG_MANIFEST_FEATURES=draco` for optional Draco support. On Linux, OpenGL remains a host SDK dependency even in vcpkg-first builds; install `libgl-dev` (or the distro equivalent). The vcpkg presets enable `LVR2_USE_SYSTEM_OPENGL=ON` for that platform SDK lookup while keeping the rest of the dependency path vcpkg-first.
...
Verified ROS/Debian dependency names were added for package-backed dependencies introduced by the modernization work, including Assimp, tl-expected (`libexpected-dev`), spdlog, TBB, TIFF, GDAL, HDF5, OpenCV, Eigen, YAML-CPP, OpenGL/GLUT, and OpenCL. Boost is no longer an LVR package/export dependency. HighFive, rply, and LASlib/LAStools remain required by the package-backed system build, but verified Lyrical/Resolute Debian package names and rosdep keys are not recorded yet; distributors may need local packages or rosdep rules for those dependencies before full system-package Debian builds pass.
